...
func discoveryHandler(request *http.Request, server *Server) HTTPResponse {
	...
	tenantIDResolved := tenant.TenantID().String()

	tenantURL := baseURL + "/" + tenantIDStr
	if isV2 {
		tenantURL = baseURL + "/" + tenantIDStr + "/v2.0"
	}

	issuer := tenantURL
	if !isV2 {
		issuer = baseURL + "/" + tenantIDResolved
	}
...
func jwksHandler(_ *http.Request, server *Server) HTTPResponse {
...
func callHomeHandler(request *http.Request, _ *Server) HTTPResponse {
	baseURL := extractBaseURL(request)
...
func buildDiscoveryDocument(tenantURL, issuer, tenant string) map[string]any {
	return map[string]any{
		...
		"token_endpoint_auth_methods_supported":      []string{"client_secret_post", "private_key_jwt", "client_secret_basic", "self_signed_tls_client_auth"},
